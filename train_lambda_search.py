...
config_template_path = "./configuration/unet_config_lambda_search.yaml"
temp_config_path = "./configuration/temp_lambda_config.yaml"
...
def create_config_with_lambda(lambda_value):
    ...
    # Create the lambda-name string representation that preserves zeros
    lambda_name = str(lambda_value).replace('.', '')
    
    # Replace all occurrences of <lambda-name> with the formatted string
    config_content = config_content.replace('<lambda-name>', lambda_name)
    
    with open(temp_config_path, 'w') as f:
        f.write(config_content)
    
    print(f"Created configuration with lambda = {lambda_value} (lambda-name = {lambda_name})")
