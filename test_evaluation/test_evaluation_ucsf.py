...
def evaluate_models_psnr(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Evaluating models")):
            undersampled_tensor, original_tensor = batch
            # Move tensors to device
            undersampled_tensor = undersampled_tensor.to(device)
            original_tensor = original_tensor.to(device)

            # Get reconstructions
            for i, model in enumerate(models):
                recon = model(undersampled_tensor)
                recon = min_max_slice_normalization(recon)
                original_tensor = min_max_slice_normalization(original_tensor)

                # Calculate PSNR for each image in the batch
                for b in range(original_tensor.shape[0]):
                    # Get individual images from batch
                    original = original_tensor[b].squeeze().cpu().numpy()
                ...
                if batch_idx < 0:
                    ...
                    undersampled = undersampled_tensor[0].cpu().numpy()
                    original = original_tensor[0].cpu().numpy()
                    reconstructed = recon[0].cpu().numpy()

                    # Print statistics
                    print(f"\nSample {batch_idx}, Model {i}:")
                    print("Degraded image stats:")
                    print(f"Shape: {undersampled.shape}")
                    print(f"Range: [{undersampled.min():.3f}, {undersampled.max():.3f}]")
                    print(f"Mean: {undersampled.mean():.3f}")
                    print(f"Std: {undersampled.std():.3f}")
                    ...
                    axes[0].imshow(undersampled[0], cmap='gray')
                    axes[0].set_title(f'Undersampled\nμ={undersampled.mean():.2f}, std={undersampled.std():.2f}')
                    axes[0].axis('off')
                        
                    axes[1].imshow(reconstructed[0], cmap='gray')
                    axes[1].set_title(f'Reconstructed\nμ={reconstructed.mean():.2f}, std={reconstructed.std():.2f}\nPSNR: {psnr:.2f}')
                    axes[1].axis('off')
                        
                    axes[2].imshow(original[0], cmap='gray')
                    axes[2].set_title(f'Original\nμ={original.mean():.2f}, std={original.std():.2f}')
                    axes[2].axis('off')
...
if __name__ == "__main__":
    ...
    evaluate_models_psnr(
        ...
        data_root="/lotterlab/users/matteo/data/UCSF-PDGM",
