...
def plot_data(csv, output_dir):
    ...
    colors = [
        cmap(i / len(values)) for i in range(len(values))
    ...
    plt.figure(figsize=figure_size)
    ax = age_sex_dist.plot(
        kind="bar", stacked=True, color=[cmap(0.2), cmap(0.7)], edgecolor="black"
    )
    ...
    plt.figure(figsize=figure_size)
    plt.hist(
        csv["OS"], bins=20, color=cmap(0.5)
    )  # Assuming OS is in months, converting to days
    ...
    scatter = plt.scatter(
        csv["Age at MRI"], csv["OS"], c=csv["Tumor Grade"], cmap=cmap, s=50
    )
    ...
    for grade in grades:
        plt.hist(
            csv[csv["WHO CNS Grade"] == grade]["OS"], bins=10, alpha=0.5, label=grade
        )
    ...
    age_status.plot(
        kind="bar", stacked=True, color=[cmap(0.7), cmap(0.2)], figsize=figure_size
    )
    ...
    plt.figure(figsize=figure_size)
    wrapped_labels = [
        textwrap.fill(label, width=20) for label in diagnosis_counts.index
    ]
    ...
    avg_survival_sex_age.plot(
        kind="bar", color=[cmap(0.3), cmap(0.7)], figsize=(10, 6), width=0.8
    )
    ...
    age_bins = pd.cut(csv["Age at MRI"], bins=[0, 3, 18, 42, 67, 96], right=False)
    grouped_data = (
        csv.groupby([age_bins, "Sex", "WHO CNS Grade"]).size().unstack(fill_value=0)
    )
    ...
    age_bins = pd.cut(csv["Age at MRI"], bins=[0, 3, 18, 42, 67, 96], right=False)
    grouped_data = (
        csv.groupby([age_bins, "Sex", "1-dead 0-alive"]).size().unstack(fill_value=0)
    )
    male_data = grouped_data.loc[(slice(None), "M"), :]
    female_data = grouped_data.loc[(slice(None), "F"), :]
    age_bin_labels = grouped_data.index.get_level_values(0).unique()
    ...
    ax.set_ylabel("Count")
    ax.set_title(
        "Stacked Bar Chart by Survival Status (1-dead 0-alive), Sex (M/F), and Age at MRI"
    )
    ...
    age_bins = pd.cut(csv["Age at MRI"], bins=[0, 3, 18, 42, 67, 96], right=False)
    grouped_data = (
        csv.groupby([age_bins, "Sex", "Final pathologic diagnosis (WHO 2021)"])
        .size()
    ...
    ax.set_ylabel("Count")
    ax.set_title(
        "Stacked Bar Chart by Pathologic Diagnosis (WHO 2021), Sex (M/F), and Age at MRI"
    )
