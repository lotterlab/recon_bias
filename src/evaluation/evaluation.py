...
def significance_test(gt, col1, col2, test_type='auto'):
    ...
    try:
        if test_type == 't-test':
            ...
            _, p_value = stats.ttest_ind(col1, col2, equal_var=False)  # Welch's t-test
            return p_value
...
def aggregate_predictions(grouped_df, classifier, groups, metric, age_bins, age_labels): 
    ...
    for group_keys, group in grouped_df:
        ...
        relevant_columns = [col for col in group_copy.columns 
                            if col in [f"{classifier['name']}_gt{appendix}",
        ...
        group_relevant.columns = group_relevant.columns.str.replace(f"{classifier['name']}_", "").str.replace(appendix, "")

        # Change gt, pred and recon 
        group_relevant = group_relevant.rename(columns={f"gt": "Ground Truth (GT)", f"pred": "Classifier on GT", f"recon": "Classifier on Reconstruction"})
