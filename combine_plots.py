    ...
    for group in group_folders:
        ...
        for category in categories:
            ...
            for file_name in os.listdir(group_path):
                if category in file_name.lower() and 'significance' not in file_name.lower():
                    base_image = os.path.join(group_path, file_name)
                elif 'significance' in file_name.lower() and category in file_name.lower():
            ...
            for metric in metrics:
                if metric in file_name.lower():
                    ...
                    if 'significance' in file_name.lower():
                        images[group_suffix][metric] = {'significance': os.path.join(reconstruction_folder, file_name)}
                    else:
                        images[group_suffix][metric] = {'base': os.path.join(reconstruction_folder, file_name)}
...
def create_classifier_plots(images, classifier_name, pdf):
    create_matrix_plot(images, classifier_name, pdf, ['predictions', 'score', 'performance'], "Classifier")

# Function to create plots for reconstruction results
def create_reconstruction_plots(images, reconstruction_name, pdf):
    create_matrix_plot(images, reconstruction_name, pdf, ['psnr', 'nrmse', 'ssim'], "Reconstruction")
...
def create_matrix_plot(images, name, pdf, categories, title_prefix):
    ...
    fig, axes = plt.subplots(num_categories * 2, num_groups, figsize=(num_groups * 4, num_categories * 6))
...
def create_gradcam_plots(images, classifier_name, pdf):
    categories = ['male_age_leq_58', 'male_age_gt_58', 'female_age_leq_58', 'female_age_gt_58']
    num_categories = len(categories)
    ...
    fig, axes = plt.subplots(nrows=num_categories, ncols=max_images_per_category, 
    ...
    for i, category in enumerate(categories):
        ...
        category_label = category.replace('male', 'Male').replace('female', 'Female').replace('age_leq_58', 'Age ≤ 58').replace('age_gt_58', 'Age > 58')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate combined PDF plots for classifiers, reconstruction, and Grad-CAM results.")
    parser.add_argument('main_folder', type=str, help='Path to the main folder containing all subfolders')
